...
os.environ['PATH'] = f"{os.environ['PATH']}:{ANDROID_HOME}/cmdline-tools/bin:{ANDROID_HOME}/cmdline-tools/latest/bin:{ANDROID_HOME}/platform-tools:{ANDROID_HOME}/build-tools"
...
"""
    ...
    try:
        ...
        return jsonify({'error': f'執行 Ant 打包失敗: {str(e)}'}), 500
    finally:
        # 清理上傳的文件和構建文件
